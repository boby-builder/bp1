...
Podaci se nalaze u osnovi mnogih web aplikacija i aplikacija za mobilne telefone. 
Neki od primera bi bili:
- Telegram, Instagram, Facebook: podatke o korisniku, njegovim prijateljima, objavama, datum rođenja...
- Google Maps: geografske koordinate lokacija na mapi, njihova imena, recenzije, datum izmene...
- E-Indeks: broj indeksa, ime, prezime, predmete, statuse predmeta, prijavljenje ispite...

Da bi se ti podaci čuvali, koriste se baze podataka. SQL je programski jezik koji omogućava programerima da upravljaju tim podacima.
...
* **Instalacija SQLLocalDB:** Prvi korak je instaliranje SQLLocalDB-a na vašem računaru. Možete preuzeti SQLLocalDB instalacione datoteke sa zvaničnog Microsoft sajta ili ih instalirati zajedno sa određenim razvojnim alatima kao što je Visual Studio (preporučljivo).
...
U aplikaciji za menadžment uglavnom je potrebno kao **Server Name** uneti `(localdb)\MyInstance`, kao i podesiti **Authentication**: `Windows Authentification`, zatim i import baze (uglavnom za našu upotrebu je to [studije](#baza-studije))
...
**SELECT** je osnovna naredba SQL jezika jer njom definišemo šta želimo. <!-- Spada u DQL naredbe (Data query language). -->
  Ona uvek ide u paru sa rečju **FROM**, kojom definišemo odakle to želimo.
<!-- Da li je informacija o DQL neophodna? -->
...
* Prikazati Imes, indeks, upisan, mesto iz tabele STUDENTI
    ```sql
    select Imes, Indeks, Upisan
    from Studenti
    ```

* Prikazati Imes, Indeks, Upisan, Datr iz tabele studenti, gde je Datr naslovljena kao 'Datum rodjenja'
    ```sql
    select Imes,Upisan,Datr as 'Datum rodjenja'
    from Studenti
    ```
* Selektovati različita mesta iz tabele Studenti, i dopisati kolonu sa NULL vrednostima
    ```sql
    select distinct *, NULL as 'Null kolona'
    from Studenti
    ```
### WHERE
* Prikazati podatke o studentima koji su upisani 2000 godine
...
* Za svako godiste studenata koje se pojavljuje u tabeli Studenti, ispisati broj studenata koji su rodjeni te godine.
    ```sql
    select datepart(year,Datr), count(*) as 'Broj po godistu'
    from Studenti
    where Datr is not null
    group by datepart(year,Datr)
    order by 'Broj po godistu'
    ```
* Sa svakog studenta (indeks, upisan) ispisati koliko ispita je polozio i njegovu prosecnu ocenu
    ```sql
    select concat(Indeks,'/',Upisan) as Indeks, avg(Ocena) as 'Prosek', count(Spred) as "Br pred"
    from prijave
...
* Koliko ima predmeta na svakoj godini?
    ```sql
    select      
    ((Semestar - 1)/ 2) + 1 as Godina,
    count(*) as BrPred
    from Planst
    group by ((Semestar - 1)/ 2) + 1
    ```
* Za svaki predmet ispisati kada je poslednji put polagan?
    ```sql
...
* Poslednji polozen predmet za svakog studenta?
    ```sql
    SELECT 
        concat(p.Indeks,'/', p.Upisan) as Student,	
        p.Spred
    FROM prij p
    WHERE rezultat = 'položio'
        AND 
    p.Datump = (
            select max(p2.Datump)
            from prij p2
            where 
                concat(p.Indeks,'/', p.Upisan) = concat(p2.Indeks,'/', p2.Upisan)
                    and 
                    p2.rezultat = 'položio'
        )
    ```     
* Za svaki ispitni rok ispisati koliko se u njemu polozilo ispita do sada?
    ```sql
    <!-- Nejasno koja tabela pruza ispitne rokove, da li to sami zaključujemo iz semestra, meseca ili nečeg trećeg -->
    ```
    ```sql
    <!--TODO probati sa mesecima -->
    select concat(datename(month,Datump),'-',datepart(year,Datump)), count(Datump)
    from prij
    where rezultat = 'položio'
    group by concat(datename(month,Datump),'-',datepart(year,Datump))
    ```
    Chatgpt solution:
    ```sql
    SELECT
    FORMAT(Datump, 'yyyy-MM') AS ispitni_rok,
    COUNT(*) AS broj_polozenih
    FROM prij
    WHERE rezultat = 'položio'
    GROUP BY FORMAT(Datump, 'yyyy-MM')
    ORDER BY ispitni_rok;
    ```

* Prikazati uspeh (prosecnu ocenu) svake generacije
    ```sql
    select Upisan, avg(Ocena) as Prosek
    from prijave
    group by Upisan
    ```
